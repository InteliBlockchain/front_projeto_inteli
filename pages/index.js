import Head from "next/head";
import Image from "next/image";
import Link from "next/link";
import { motion } from "framer-motion";

import rightArrow from "../assets/icons/arrow-right.svg";

const Home = () => {
	return (
		<div>
			<Head>
				<title>Inteli Blockchain - API Docs</title>
				<meta
					name="description"
					content="Inteli Blockchain API Documentation"
				/>
				<meta name="viewport" content="width=device-width, initial-scale=1.0" />
				<link rel="icon" href="/favicon.ico" />
			</Head>

			<main className="container flex-1 px-4 py-6 justify-center items-center min-w-full w-full h-screen">
				<motion.div
					className="xs:grid-cols-1
					md:w-1/2
					xl:w-1/3 xl:gap-3
          mx-auto grid gap-x-4 my-auto h-full items-center justify-center"
				>
					<div className="grid-cols-11 md:grid-cols-8  justify-center items-center">
						<div className="mx-auto mb-24">
							<h3 className="text-2xl font-bold text-center mb-0">
								Documentação da API
							</h3>
							<h1 className="text-4xl font-bold text-center mb-2">
								Inteli Blockchain
							</h1>
							<p className="text-xl text-center">
								Projeto Blockchain para o Inteli que registra uma carteira para
								cada aluno e mantêm registro de suas atividades no Campus.
							</p>
						</div>

						<motion.div className="text-lg md:text-xl p-4 bg-gray-200 rounded-lg shadow-lg">
							<span className="text-red-500 mx-auto">
								GET {`{{url}}`}/api/v1/blockchain/status
								<br />
								POST {`{{url}}`}/api/v1/blockchain/status
								<br />
								PUT {`{{url}}`}/api/v1/blockchain/status
								<br />
								DELETE {`{{url}}`}/api/v1/blockchain/status
							</span>
						</motion.div>
					</div>

					<div className="flex items-center justify-center">
						<a href="https://google.com">
							<motion.button
								className="bg-black text-white font-bold py-2 px-4 rounded-md mx-auto flex justify-between shadow-md"
								whileHover={{
									scale: 1.1,
									y: -2.5,
									transition: { duration: 0.2 },
								}}
							>
								<span className="mr-2">Início da documentação</span>
								<Image
									className="ml-2"
									src={rightArrow}
									height={24}
									width={24}
								/>
							</motion.button>
						</a>
					</div>
				</motion.div>
			</main>
		</div>
	);
};

export default Home;
